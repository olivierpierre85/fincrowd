...
function of_option_setup()	
{
	...
	$options_machine = new Composer_Options_Machine($of_options);
...
function of_save_options($data, $key = null) {
	...
	composer_generate_options_css($data);
...
function composer_generate_options_css($newdata) {

	$data = $newdata;
	$GLOBALS["amz_option_data"] = $data;

	$uploads = wp_upload_dir();
	$css_dir = COMPOSER_ADMIN . '/_css/';
	...
	if( ! $wp_filesystem->is_dir( $pix_uploads_dir . '/' . $theme_name . '/' ) ) {
		$wp_filesystem->mkdir( $pix_uploads_dir . '/' . $theme_name . '/' );
	}
