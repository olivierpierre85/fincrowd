...
add_shortcode( 'wpneo_crowdfunding_form','wpneo_shortcode_croudfunding_form' ); // Shortcode for Campaign Submit Form
        ...
        function wpneo_shortcode_croudfunding_form( $atts ){
            ...
            if(isset($_GET['action'])){
                if($_GET['action']=='edit'){
                    $post_id = intval(sanitize_text_field($_GET['postid']));
            ...
            if (WPNEO_CROWDFUNDING_TYPE == 'free') {
                ...
                $html .= '<p> <i> ' . __('pro version is required to add more than 1 reward', 'wp-crowdfunding') . '. <a href="https://wpneo.com/wp-crowdfunding" target="_blank">' . __('click here to get pro version', 'wp-crowdfunding') . '</a> </i></p>';
            }else {
