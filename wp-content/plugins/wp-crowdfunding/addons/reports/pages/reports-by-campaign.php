...
$reports                = new Wpneo_Crowdfunding_Reports_Query();
...
<div class="wrap campaign-warp">
    <!-- Top menu -->
    <a href="<?php echo admin_url( 'admin.php?page=wpneo-crowdfunding-reports&tab=top_campaigns&date_range=last_7_days' ); ?>" class="page-title-action"><?php _e('Last 7 days', 'wp-crowdfunding') ?></a>
    <a href="<?php echo admin_url( 'admin.php?page=wpneo-crowdfunding-reports&tab=top_campaigns&date_range=this_month' ); ?>" class="page-title-action"><?php _e('This Month', 'wp-crowdfunding') ?></a>
    <a href="<?php echo admin_url( 'admin.php?page=wpneo-crowdfunding-reports&tab=top_campaigns&date_range=last_month' ); ?>" class="page-title-action"><?php _e('Last Month', 'wp-crowdfunding') ?></a>
    <a href="<?php echo admin_url( 'admin.php?page=wpneo-crowdfunding-reports&tab=top_campaigns&date_range=last_6_months' ); ?>" class="page-title-action"><?php _e('Last 6 Months', 'wp-crowdfunding') ?></a>
    <a href="<?php echo admin_url( 'admin.php?page=wpneo-crowdfunding-reports&tab=top_campaigns&date_range=this_year' ); ?>" class="page-title-action"><?php _e('This Year', 'wp-crowdfunding') ?></a>
    <a href="<?php echo admin_url( 'admin.php?page=wpneo-crowdfunding-reports&tab=top_campaigns&date_range=last_year' ); ?>" class="page-title-action"><?php _e('Last Year', 'wp-crowdfunding') ?></a>

    <form method="get" action="">
        <input type="hidden" name="page" value="wpneo-crowdfunding-reports" />
    ...
    if ( $loop->have_posts() ) {
        ...
        <table class="wp-list-table widefat fixed striped">
            ...
            while ( $loop->have_posts() ) : $loop->the_post();
                ...
                $total_sales_by_product     = $reports->wpneo_crowdfunding_get_order_report_data($get_sales_query_by_item);
                $total_sales_by_count       = $reports->wpneo_crowdfunding_get_order_report_data($get_sales_query_by_item_count);
    ...
    <a class="button-default" href="<?php echo add_query_arg('export_csv', str_replace('"', '--', serialize($csv)),$_SERVER['REQUEST_URI']); ?>"><?php _e( "Export CSV","wp-crowdfunding" ); ?></a>
</div>